...
const createStore = () => {
  ...
  const switch1193Provider = async ({
    ...
    if (!chainId2) {
      chainId2 = alwaysNumber((await get("provider").getNetwork()).chainId);
    }
    const signer2 = get("provider").getSigner(addressOrIndex);
    try {
  ...
  const chainChangedHandler = (eipProvider, addressOrIndex) => (chainId2) => set1193Provider(eipProvider, addressOrIndex, alwaysNumber(chainId2));
  ...
  const set1193Provider = async (eipProvider, addressOrIndex, chainId2) => {
    init();
    let accounts;
    try {
    ...
    return switch1193Provider({ addressOrIndex, chainId: chainId2 });
  };
  const setProvider = async (provider2, addressOrIndex = 0) => {
    ...
    const { chainId: chainId2 } = await provider2.getNetwork();
    let signer2, signerAddress2;
    if (addressOrIndex !== null) {
      try {
        if (typeof provider2.listAccounts === "function") {
          signer2 = provider2.getSigner(addressOrIndex);
        } else {
...
const createContractStore = () => {
  const { emit, get, subscribe, assign, deleteAll } = proxied();
  const attachContract = async (name, address2, abi2, fromSigner = true) => {
    assign({
      [name]: [address2, abi2, fromSigner]
    });
    emit();
  };
  return {
...
const abi = [
    ...
    inputs: [
      {
        ...
        internalType: "struct ReaperGambitEpitaph.Epitaph",
        name: "ep",
        type: "tuple"
      }
    ],
  ...
  },
  {
    ...
    outputs: [
      {
        internalType: "contract BMPImage",
        name: "",
        type: "address"
      }
    ],
  ...
  },
  {
    ...
    outputs: [
      {
        internalType: "contract IERC20ReaperGambit",
        name: "",
        type: "address"
      }
    ],
  ...
  },
  {
    ...
    outputs: [
      ...
      },
      {
        ...
        internalType: "struct Pricing.Coupon",
        name: "coupon",
        type: "tuple"
      }
    ],
